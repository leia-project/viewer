...
import {
  ...
  function computeAttributes(options, extraAttributes: Array<{ param: string, value: number }>) {
	const vertexFormat = options.vertexFormat;
	const geometry = options.geometry;
	const shadowVolume = true;//options.shadowVolume;
	  ...
	  for (let i = 0; i < length; i += 3) {
		...
		if (vertexFormat.st) {
		  if (!defined(flatTexcoords)) {
			...
			const stx = Cesium.Math.clamp(st.x / boundingRectangle.width, 0, 1);
			const sty = Cesium.Math.clamp(st.y / boundingRectangle.height, 0, 1);
			if (bottom) {
		...
		if (
		  ...
		  if (wall) {
			if (i + 3 < length) {
			  ...
			  if (Cartesian3.equalsEpsilon(p1, position, Cesium.Math.EPSILON10)) {
...
// SDG: For offsets, I also want an extrude direction for the bottom (inverse direction...)
	...
	geometry.attributes.vectorUp = new GeometryAttribute({
		componentDatatype: ComponentDatatype.FLOAT,
		componentsPerAttribute: 3,
		values: extrudeNormals,
	});
	  ...
	  if (extraAttributes) {
		extraAttributes.forEach((attr) => {
			geometry.attributes[attr.param] = new GeometryAttribute({
				componentDatatype: ComponentDatatype.FLOAT,
   ...
   * @param {number} [options.granularity=Cesium.Math.RADIANS_PER_DEGREE] The distance, in radians, between each latitude and longitude. Determines the number of positions in the buffer.
  ...
  function expandRectangle(
	...
	const lonAdjusted =
	  longitude >= 0.0 ? longitude : longitude + Cesium.Math.TWO_PI;
  ...
  PolygonGeometry.computeRectangleFromPositions = function (
	...
	if (result.east - result.west > polygon.eastOverIdl - polygon.westOverIdl) {
	  ...
	  if (result.east > Cesium.Math.PI) {
		result.east = result.east - Cesium.Math.TWO_PI;
	  }
	  if (result.west > Cesium.Math.PI) {
		result.west = result.west - Cesium.Math.TWO_PI;
	  }
	}
  ...
  function getTangentPlane(rectangle, positions, ellipsoid) {
	if (rectangle.height >= Cesium.Math.PI || rectangle.width >= Cesium.Math.PI) {
		...
		if (rectangle.south < 0 && rectangle.north > 0) {
		  ...
		  for (let i = 0; i < positions.length; ++i) {
			...
			results[i] = new Cartesian2(
			  cartographic.longitude / Cesium.Math.PI,
			  cartographic.latitude / Cesium.Math.PI_OVER_TWO,
			);
		  }
	...
	if (rectangle.height >= Cesium.Math.PI || rectangle.width >= Cesium.Math.PI) {
	  return (position, result) => {
		...
		if (rectangle.south < 0 && rectangle.north > 0) {
		  ...
		  result.x = cartographic.longitude / Cesium.Math.PI;
		  result.y = cartographic.latitude / Cesium.Math.PI_OVER_TWO;
  ...
  function createSplitPolygons(rectangle, ellipsoid, arcType, perPositionHeight) {
	return (polygons, results) => {
	  if (
		!perPositionHeight &&
		(rectangle.height >= Cesium.Math.PI_OVER_TWO ||
		  rectangle.width >= 2 * Cesium.Math.PI_OVER_THREE)
	  ) {
   ...
   */
  PolygonGeometry.createGeometry = function (polygonGeometry, extraAttributes: Array<{ param: string, value: number }>) {
	...
	const extrude =
	  ...
	  !Cesium.Math.equalsEpsilon(height, extrudedHeight, 0, Cesium.Math.EPSILON2);
	...
	if (extrude) {
	  ...
	  for (i = 0; i < polygons.length; i++) {
		...
		if (closeTop || closeBottom) {
		  options.wall = false;
		  topAndBottom.geometry = computeAttributes(options, extraAttributes);
		...
		for (let k = 0; k < walls.length; k++) {
		  ...
		  wall.geometry = computeAttributes(options, extraAttributes);
	  ...
	  for (i = 0; i < polygons.length; i++) {
		...
		geometryInstance.geometry = computeAttributes(options, extraAttributes);
	...
	rectangle: {
	  get: function () {
		if (!defined(this._rectangle)) {
		  ...
		  this._rectangle = Cesium.PolygonGeometry.computeRectangleFromPositions(
			positions,
			this._ellipsoid,
			this._arcType,
		  );
