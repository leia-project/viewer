...
- **`zonalStatistics` settings:** `zoneLayerId` is required and identifies the clickable zone geometry layer; `layers` is required and maps themed data layers via `{ id, attribute }` entries; `zoneCodeAttribute` defaults to `postcode`; optional `targetLabelAttribute`, `labelTooltipAttribute`, and `targetLabelTooltipAttribute` provide target-label and per-cell tooltip data in the passport table.
