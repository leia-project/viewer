...
- **`zonalStatistics` settings:** `zoneLayerId` is required and identifies the clickable zone geometry layer; `layers` is required and maps themed data layers via `{ id, attribute }` entries; `zoneCodeAttribute` defaults to `postcode`; `globeOpacity` is clamped to `0..100` (default `50`); optional `targetLabelAttribute`, `labelTooltipAttribute`, and `targetLabelTooltipAttribute` provide target-label and per-cell tooltip data in the passport table.
...
- **GeoJson categorical style naming.** The strict naming convention is `style: "category"` for category-based string styling, and `settings.categoryColors` for deterministic value-to-color maps. `label`/`labelColors` are no longer supported naming in shipped configs.
