...
- **Geocoder providers.** The header search (`src/lib/components/header/HeaderUtilityGeocoder/`) is provider-based: each backend implements `GeocoderProvider` (`id`, `defaultUrl`, `labelKey`, optional `requiresDebounce`, `suggest()`, `resolveBounds()`) in `geocoder-providers.ts`; the Svelte component only orchestrates fetching, merging, and the camera fly-to. Add new backends to the `geocoderProviders` registry — do not add branching to the component. Config `tools[id=geocoder].settings.name` accepts a single provider name **or an array** (e.g. `["locatieserver", "geolocation", "geoportail-lu"]` for NL + BE + LU); results are merged round-robin (5 per provider, 10 total) with providers that return nothing yielding their slots, and each result shows its source via the `tools.geocoder.providers.*` i18n label. `settings.url` overrides the endpoint only when exactly one provider is configured.
