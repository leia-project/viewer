...
- **`zonalStatistics` settings:** supports `globeOpacity` (0..100, default `50`) to control globe translucency while the tool is active, plus optional `labelTooltipAttribute` and `targetLabelTooltipAttribute` to show per-cell tooltip text in the passport table.
